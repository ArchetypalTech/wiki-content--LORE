...
The first condition is that the `PLAYER` must be in posession of the the `Ebony Sword`.
...
On the `Red Dragon` entity, we add:

- one `Condition`
- three `Effects`
- one `Action`

And we add them to the `Dragon` as he is the target entity and to have the `Action elements` grouped as much as possible for easy setup.
...
- `Effect.text_value`: **Value**: *true*, **Idx**: *0* or you can leave empty 
...
- **Value**: *Its scales are almost impregnable. Its a miracle that this dragon has been defeated* 
- **Idx**: `2`
...
The last step on the `Action System` is to unite the `Trigger`, `Condition/s` and `Effect/s`. Add `Action` to the `Red Dragon`
...
- use the sword on the dragon should defeat it, allowing: 
	- the dragon can be check and looted
	- the dragon can be touched now
